...
import { submitTeacherReport } from "../../actions"
...
export function ObservationDialog({
  ...
  // For observation, teacher assesses all members
  const filteredMembers = React.useMemo(() => {
    return members // Teacher assesses all members, no self-filtering needed
  }, [members])
  ...
  const handleSubmit = async () => {
    ...
    if (answers.length !== filteredMembers.length || answers.some((v) => !v)) {
      setError("Harap nilai semua anggota kelompok sebelum menyimpan.")
      return
    }
    setIsSubmitting(true)
    try {
      // Submit one by one for each member
      ...
      toast.success("Penilaian observasi berhasil disimpan!")
      onOpenChange(false)

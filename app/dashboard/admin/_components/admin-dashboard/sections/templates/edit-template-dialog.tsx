...
import { Plus, Trash2, Copy, ArrowLeft, ArrowRight, Check, X, Edit } from "lucide-react"
...
export function EditTemplateDialog({
  ...
  useEffect(() => {
    if (template && open) {
      // Group existing stage configs by stage name
      ...
      const sortedStages = Object.values(stageGroups).sort((a: any, b: any) => {
        const aOrder = Math.min(...template.stageConfigs
          .filter(c => c.stageName === a.stageName)
          .map(c => c.displayOrder))
        const bOrder = Math.min(...template.stageConfigs
          .filter(c => c.stageName === b.stageName)
          .map(c => c.displayOrder))
  ...
  const onSubmit = async (values: TemplateFormData) => {
    ...
    try {
      ...
      const result = await updateTemplate(template!.id, transformedData)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-sm text-muted-foreground">
                Update template configuration and assessment stages
              </DialogDescription>
            </div>
          </div>
          <Button
